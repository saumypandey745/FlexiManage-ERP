...
describe("Organization Services", () => {
  ...
  describe("BranchService", () => {
    it("should create a branch and log audit", async () => {
      ...
      const result = await branchService.createBranch("tenant-1", "user-1", {
        code: "BR-1",
        name: "HQ",
      });

      expect(result).toEqual(mockBranch);
      expect(repo.createBranch).toHaveBeenCalledWith("tenant-1", {
        code: "BR-1",
        name: "HQ",
      });
  ...
  describe("DepartmentService", () => {
    it("should create a department and log audit", async () => {
      ...
      const result = await departmentService.createDepartment(
        "tenant-1",
        "user-1",
        { code: "DEP-1", name: "Engineering" }
      );

      expect(result).toEqual(mockDept);
      expect(repo.createDepartment).toHaveBeenCalledWith("tenant-1", {
        code: "DEP-1",
        name: "Engineering",
      });
      expect(prisma.auditLog!.create).toHaveBeenCalled();

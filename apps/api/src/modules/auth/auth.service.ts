...
export class AuthService {
  ...
  async login(dto: LoginDto, ipAddress: string) {
    ...
    if (attempts >= 5) {
      throw new BaseException('Account locked due to multiple failed login attempts. Please try again later.', 'AUTH-429', 429);
    }
  ...
  async resetPassword(dto: ResetPasswordDto) {
    ...
    if (!userId) {
      throw new BaseException('Invalid or expired password reset token', 'AUTH-400', 400);
    }
  ...
  async verifyEmail(dto: VerifyEmailDto) {
    ...
    if (!userId) {
      throw new BaseException('Invalid or expired verification token', 'AUTH-400', 400);
    }

...
export class TaskService {
  ...
  async create(tenantId: string, userId: string, dto: CreateTaskDto) {
    ...
    if (existingCode) {
      throw new BadRequestException(`Task code ${dto.code} already exists in project`);
    }
  ...
  async update(tenantId: string, userId: string, id: string, dto: UpdateTaskDto) {
    ...
    if (dto.status === 'DONE' && task.status !== 'DONE') {
      ...
      if (blockingDependencies.length > 0) {
        throw new BadRequestException('Cannot complete task due to uncompleted blocking dependencies');
      }
    }
  ...
  async assign(tenantId: string, userId: string, id: string, dto: AssignTaskDto) {

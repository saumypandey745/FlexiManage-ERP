...
export class EmailService {
  ...
  async sendWelcomeEmail(email: string, firstName: string): Promise<void> {
    try {
      ...
      this.logger.error(`[EmailService] Failed to send Welcome Email to ${email}`, error.stack);
    }
  }
  ...
  async sendVerifyEmail(email: string, token: string): Promise<void> {
    ...
    try {
      ...
      this.logger.error(`[EmailService] Failed to send Verify Email to ${email}`, error.stack);
    }
  }
  ...
  async sendForgotPassword(email: string, token: string): Promise<void> {
    ...
    try {
      ...
      this.logger.error(`[EmailService] Failed to send Forgot Password to ${email}`, error.stack);
    }
  }
  ...
  async sendPasswordResetSuccess(email: string): Promise<void> {
    try {
      ...
      this.logger.error(`[EmailService] Failed to send Password Reset Success to ${email}`, error.stack);
    }
  }
  ...
  async sendInviteUser(email: string, inviterName: string, companyName: string, token: string): Promise<void> {
    ...
    try {
      ...
      this.logger.error(`[EmailService] Failed to send Invite Email to ${email}`, error.stack);
    }
  }
  ...
  async sendGenericEmail(to: string, subject: string, html: string): Promise<void> {
    try {
      ...
      this.logger.error(`[EmailService] Failed to send Generic Email to ${to}`, error.stack);
    }
  }
}

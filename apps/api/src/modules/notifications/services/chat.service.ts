...
export class ChatService {
  ...
  async sendMessage(tenantId: string, senderId: string, dto: SendMessageDto) {
    ...
    if (!participant) throw new NotFoundException('User is not a participant in this room');
  ...
  async getMessages(tenantId: string, roomId: string, userId: string, page = 1, limit = 50) {

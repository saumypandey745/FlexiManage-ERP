...
export class InvoiceService {
  ...
  async send(tenantId: string, id: string, actionUserId: string) {
    ...
    if (!invoice) throw new BaseException('Invoice not found', 'FIN-INV-404', 404);
    if (invoice.status !== InvoiceStatus.DRAFT) throw new BaseException('Invoice is not a draft', 'FIN-INV-400', 400);

    const updated = await this.repository.updateStatus(tenantId, id, InvoiceStatus.SENT);

...
export class LeaveRepository {
  ...
  async createLeave(tenantId: string, employeeId: string, dto: CreateLeaveDto) {
    ...
    if (overlap) {
      throw new BaseException('Leave request overlaps with existing leave', 'HRMS-LEV-409', 409);
    }
  ...
  async updateStatus(tenantId: string, id: string, approverId: string, dto: ApproveLeaveDto) {

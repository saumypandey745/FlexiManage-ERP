...
export class CustomerController {
  ...
  async createCustomer(@CurrentUser() user: any, @Body() dto: CreateCustomerDto) {
    return this.customerService.createCustomer(user.tenantId, user.id, dto);
  }

  @Patch(':id')
  @ApiOperation({ summary: 'Update Customer' })
  async updateCustomer(@CurrentUser() user: any, @Param('id') id: string, @Body() dto: UpdateCustomerDto) {

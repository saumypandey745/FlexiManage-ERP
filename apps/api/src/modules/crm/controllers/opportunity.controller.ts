...
export class OpportunityController {
  ...
  async updateOpportunity(@CurrentUser() user: any, @Param('id') id: string, @Body() dto: UpdateOpportunityDto) {
  ...
  async winOpportunity(@CurrentUser() user: any, @Param('id') id: string) {
    return this.opportunityService.setStage(user.tenantId, id, user.id, OpportunityStage.CLOSED_WON);
  }

  @Patch(':id/lose')
  @ApiOperation({ summary: 'Mark Opportunity as Lost' })
  async loseOpportunity(@CurrentUser() user: any, @Param('id') id: string) {
    return this.opportunityService.setStage(user.tenantId, id, user.id, OpportunityStage.CLOSED_LOST);
  }
}

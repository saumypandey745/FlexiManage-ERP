...
describe("RbacService", () => {
  ...
  it("should assign a role to a user and invalidate cache", async () => {
    ...
    (cache.invalidateUserPermissions as jest.Mock).mockResolvedValueOnce(
      undefined
    );
  ...
  it("should assign a permission to a role and invalidate cache", async () => {
    ...
    (prisma.auditLog!.create as jest.Mock).mockResolvedValueOnce({});
    (prisma.userRole!.findMany as jest.Mock).mockResolvedValueOnce([
      { userId: "u1" },
    ]);
    (cache.invalidateRolePermissions as jest.Mock).mockResolvedValueOnce(
      undefined
    );
    ...
    expect(repo.assignPermissionToRole).toHaveBeenCalledWith(
      "role-1",
      "perm-1"
    );
    expect(prisma.auditLog!.create).toHaveBeenCalled();
    expect(cache.invalidateRolePermissions).toHaveBeenCalledWith("role-1", [
      "u1",
    ]);
  });
});
